...
#[derive(Template)]
#[template(path = "proposer_stats.html")]
struct ProposerStatsTemplate {
    bolt_proposers: Vec<Sidecar>,
    interstate_proposers: Vec<Sidecar>,
    current_slot: String,
}

async fn stats_handler(State(proposer_router): State<Arc<ProposerRouter>>) -> Html<String> {
    ...
    let template = ProposerStatsTemplate {
        bolt_proposers,
